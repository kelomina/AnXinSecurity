      ...
      <div className="scan-controls card">
        <div className="scan-actions">
          ...
          <button
            ...
            className={`btn btn-primary scan-action ${isScanning ? 'btn-warning' : ''}`}
          >
      ...
      {selectedFiles.length > 0 && !isScanning && (
        <div className="card" style={{ marginBottom: '1rem' }}>
          <div className="section-header">
            ...
            <button onClick={clearSelection} className="btn btn-outline-secondary btn-sm">
              <X size={16} />
              清除选择
            </button>
          </div>
          <div className="file-list" style={{ maxHeight: '200px', overflowY: 'auto' }}>
            {selectedFiles.slice(0, 10).map((path, index) => (
              <div key={index} className="file-item" style={{ padding: '0.5rem', borderBottom: '1px solid var(--border-color)' }}>
                <span className="file-path" style={{ fontSize: '0.9rem' }}>{path}</span>
              </div>
            ))}
            {selectedFiles.length > 10 && (
              <p className="text-muted" style={{ padding: '0.5rem', textAlign: 'center' }}>
      ...
      {error && (
        <div className="card" style={{ marginBottom: '1rem', borderColor: 'var(--color-danger)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--color-danger)' }}>
            ...
            <span>{error}</span>
            <button
              onClick={() => useScannerStore.setState({ error: null })}
              className="btn-icon"
