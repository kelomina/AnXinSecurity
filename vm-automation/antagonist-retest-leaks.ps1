# antagonist-retest-leaks.ps1 - run remaining leaked samples (seq 2/3/4 from the
# retest manifest) one at a time with session recovery per sample.
# Reads sample paths from the retest CSV (UTF-8) to avoid hardcoded CJK literals.
param(
    [string]$VmId = '7b66415c-52cb-468e-b9bd-368746f42863',
    [int]$ObserveSec = 30,
    [string]$Manifest = 'E:\Project\HTML\AnXinSecurity\vm-automation\output\sample-retest-leaks.csv',
    [string]$SkipSeqs = '1'   # case 1 already verified STRONG-INTERCEPT
)
$ErrorActionPreference = 'Continue'
function W([string]$m) { Write-Output $m }
"=== retest-leaks @ $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') ==="
...
$skip = @($SkipSeqs -split ',' | ForEach-Object { [int]$_.Trim() })
$samples = @($all | Where-Object { $skip -notcontains [int]$_.Seq })
...
foreach ($smpl in $samples) {
    ...
    if (-not (Test-Path -LiteralPath $src)) { W "  SRC-MISSING: $src"; continue }

    $s = Connect-VM
    if (-not $s) { W "  CONNECT-FAIL"; continue }

    # pre-clean + service reset (reset interception queue)
    ...
    $launch = Invoke-Command -Session $s -ScriptBlock { param($p) Start-Process -FilePath $p -PassThru -ErrorAction SilentlyContinue } -ArgumentList $vmFile -ErrorAction SilentlyContinue
    if (-not $launch -or -not $launch.Pid) {
        W "  LAUNCH-FAIL (Defender blocked?)"
        $results += [pscustomobject]@{ Seq=$seq; Hash=$hash; Verdict='LAUNCH-FAIL'; Note='Start-Process returned no PID' }
        Remove-PSSession $s; continue
    }
    ...
    $s = Connect-VM
    if (-not $s) {
        W "  RECONNECT-FAIL after observe"
        $results += [pscustomobject]@{ Seq=$seq; Hash=$hash; Verdict='RECONNECT-FAIL'; Note='session broken after sample launch' }
        continue
    }
    ...
    $diagNew = Invoke-Command -Session $s -ScriptBlock {
        ...
        $lines = Get-Content $f | Select-Object -Skip $base
        $out = @()
        foreach ($ln in $lines) {
            try { $o = $ln | ConvertFrom-Json; $out += $o } catch {}
        }
        ,$out
    ...
    if ($diagSummary.Count -eq 0 -and $diagNew.Count -gt 0) {
        $diagSummary = $diagNew | Select-Object -First 3 | ForEach-Object { "[$($_.stage)] pid=$($_.payload.pid) $($_.payload.reason)" }
    }
    ...
    } -ArgumentList $launchedPid, $vmCase -ErrorAction SilentlyContinue
