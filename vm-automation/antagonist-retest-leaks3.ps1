# antagonist-retest-leaks3.ps1 - run leaked samples (seq 2/3/4) using padded copies
# (append 1 byte to change hash, bypassing Defender's cached detection on the ORIGINAL
# hash while keeping the PE runnable). Original files are never modified.
param(
...
"=== retest-leaks3 @ $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') ==="
...
foreach ($smpl in $samples) {
    ...
    # copy sample then pad 1 byte at end (in VM) to change hash
    try {
        ...
        # pad: append a single 0x00 byte
        Invoke-Command -Session $s -ScriptBlock { param($f) $fs=[System.IO.File]::Open($f,[System.IO.FileMode]::Append); $fs.WriteByte(0); $fs.Close(); (Get-Item $f).Length } -ArgumentList $vmFile -ErrorAction SilentlyContinue | ForEach-Object { W "  padded, new size=$_" }
    ...
    if (-not $launch -or -not $launch.Pid) {
        W "  LAUNCH-FAIL (still blocked)"
        $results += [pscustomobject]@{ Seq=$seq; Verdict='LAUNCH-FAIL'; Note='padded copy still blocked by Defender' }
