...
- **禁止使用 kd/cdb/WinDbg 进行内核调试或 dump 分析**（含 `kd -kl` 本地内核调试）：符号服务器下载会占满宿主内存导致系统卡死（2026-08 两次复现）。内核 DbgPrint 捕获一律使用 Sysinternals DebugView64（零符号下载）；驱动诊断优先走驱动内建 Trace 环形缓冲 + IOCTL 查询通道。
