...
pub fn run_service() -> Result<(), String> {
    ...
    let runtime_guard =
        match start_protection_runtime(status_handle.clone(), started_at, stop_flag.clone()) {
            Ok(guard) => guard,
            Err(e) => {
...
pub(crate) fn resolve_nt_path(path: &std::path::Path) -> Result<String, String> {
    ...
    // Get NT path using GetFinalPathNameByHandle with VOLUME_NAME_NT
...
/// 向驱动显式注册 UI 进程 PID，确保内核级进程保护生效。
///  Explicitly register the UI process PID with the driver for kernel-level protection.
fn register_ui_process_pid(pid: u32) {
    use crate::utils::driver_client::DriverClient;
