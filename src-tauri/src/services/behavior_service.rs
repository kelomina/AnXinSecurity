...
impl BehaviorService {
    ...
    pub async fn ingest_event(&self, event: serde_json::Value) -> Result<(), String> {
        ...
        let process_name = event.get("processName").and_then(|v| v.as_str()).unwrap_or("").to_string();
        let operation = event.get("operation").and_then(|v| v.as_str()).unwrap_or("").to_string();
    ...
    pub async fn list_events(&self, pid: Option<u64>, limit: u64) -> Result<Vec<serde_json::Value>, String> {
        let pool = self.pool.lock().await;
        ...
        let result: Vec<serde_json::Value> = rows
            ...
            .map(|row| {
                ...
                let mut event: serde_json::Value = serde_json::from_str(&details).unwrap_or(serde_json::json!({}));
