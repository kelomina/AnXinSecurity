...
pub async fn execute_exit(app_handle: AppHandle, keep_service: bool) -> Result<(), String> {
    ...
    tauri::async_runtime::spawn(async move {
        ...
        if let Err(e) = execute_exit_after_invoke_response(app_handle_for_exit, keep_service).await
        {
            eprintln!("[TrayExit] Exit flow failed: {}", e);
        }
    });
...
pub async fn execute_exit_after_invoke_response(
    ...
    if !keep_service {
        // 拆分架构：先请求后端服务进程优雅停止（幂等；服务未连接时静默跳过，
        // 兼容单 exe standalone 场景）。
        //  Split architecture: first request a graceful stop of the backend service
        //  process (idempotent; silently skipped when not connected, covering the
        //  legacy single-exe standalone scenario).
        if let Some(bridge) = app_handle.try_state::<Arc<IpcBridgeService>>() {
            if bridge.is_connected() {
                if let Err(e) = bridge.request(ipc_methods::SHUTDOWN_SERVICE, serde_json::json!({}))
                {
                    eprintln!("[TrayExit] shutdown_service request failed: {}", e);
                }
            }
        }
...
/// 函数名称：close_main_window
/// 函数作用：主窗口关闭 = 退出 Main 进程本身（拆分架构）。后台防护由独立服务进程承担，
///          托盘进程继续驻留；keepService 语义不适用于窗口关闭。
/// Purpose: Closing the main window exits the Main process itself (split architecture).
///          Background protection is owned by the dedicated service process and the tray
///          process stays resident; keepService semantics do not apply here.
/// 调用方：前端标题栏关闭按钮
/// Called by: Frontend titlebar close button
#[tauri::command]
pub async fn close_main_window(app_handle: AppHandle) -> Result<(), String> {
    ...
    tauri::async_runtime::spawn(async move {
        // 与 execute_exit 相同的约束：等 invoke 响应回程后再销毁 WebView。
        //  Same constraint as execute_exit: destroy the WebView only after the
        //  invoke response has made it back.
        tokio::time::sleep(Duration::from_millis(75)).await;
        let _ = app_handle_for_close.emit("app-exiting", ());
        destroy_webview_windows_for_exit(&app_handle_for_close);
        tokio::time::sleep(Duration::from_millis(150)).await;
