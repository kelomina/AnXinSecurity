...
/// 函数名称：lookup_rust_side_text
/// 函数作用：供 Rust 侧用户可见文本（托盘菜单等）按 locale 查找翻译；任何失败返回 None，
///          由调用方回退到安全默认文案。
/// Function name: lookup_rust_side_text
/// Purpose: Looks up a translation for Rust-side user-visible text (tray menu etc.) by
///          locale; returns None on any failure so callers can fall back to safe defaults.
pub fn lookup_rust_side_text(locale: &str, key: &str) -> Option<String> {
