...
pub trait InterceptionProcessControl: Send + Sync {
    ...
    fn terminate_process(&self, pid: u32) -> Result<bool, String>;
}
...
impl InterceptionProcessControl for NoopInterceptionProcessControl {
    ...
    fn terminate_process(&self, _pid: u32) -> Result<bool, String> {
        Ok(true)
    }
}
...
pub struct InterceptionService {
    ...
    /// headless 自动终止模式（测试/无 UI 服务场景专用）。开启后，block 拦截
    /// 因无 UI 客户端无法弹窗时直接终止进程，而非回滚放行。
    ///  Headless auto-terminate mode (test / headless service scenario only).
    headless_auto_terminate: AtomicBool,
}
...
impl InterceptionService {
    ...
    fn new_with_process_control_internal(
        ...
        Self {
            ...
            persist_suspensions,
            process_control,
            headless_auto_terminate: AtomicBool::new(false),
        }
    }

    /// 启用/关闭 headless 自动终止模式（测试/无 UI 服务场景专用）。
    ///  Toggle headless auto-terminate mode (test / headless service scenario only).
    pub fn set_headless_auto_terminate(&self, enabled: bool) {
        self.headless_auto_terminate.store(enabled, Ordering::Relaxed);
    }

    /// 当前是否处于 headless 自动终止模式。
    ///  Whether headless auto-terminate mode is active.
    fn headless_auto_terminate_enabled(&self) -> bool {
        self.headless_auto_terminate.load(Ordering::Relaxed)
    }
    ...
    /// 拦截窗口建不出来时的回滚：默认恢复目标进程并清理全部簿记；开启
    /// headless 自动终止模式时，直接终止目标进程（fail-closed）而不是放行。
    ///  Rollback when the interception window could not be created: by default
    ///  resumes the target process and clears all bookkeeping; in headless
    ///  auto-terminate mode it terminates the process (fail-closed) instead.
    ///
    ...
    /// 与 `enqueue_internal` 里台账写入失败的回滚一致，默认选择恢复进程并保留告警与诊断记录：
    /// “记录在案的放行”优于“用户永远无法被询问的冻结”。
    ///  Trade-off: staying suspended without a user decision freezes the process forever with
    /// no recovery path. Mirrors the ledger-failure rollback in `enqueue_internal` — by default
    /// resume the process and keep the alert. A logged release beats a freeze nobody can be asked
    /// about.
    ///
    /// headless 自动终止模式是此取舍的例外：测试/无 UI 服务场景要求 block 拦截真的把
    /// 威胁进程干掉，否则恶意样本会被放行逃逸，因此改为终止进程（fail-closed）。
    ///  Headless auto-terminate mode is the exception to this trade-off: test / headless
    ///  service scenarios require a block intercept to actually kill the threat, otherwise
    ///  the malicious sample escapes, so we terminate instead (fail-closed).
    fn rollback_failed_show(&self, entry: InterceptionEntry) -> Option<InterceptionEntry> {
        if self.headless_auto_terminate_enabled() {
            return self.headless_auto_terminate(entry);
        }
        if let Err(resume_err) = self.process_control.resume_process(entry.pid) {
            append_interception_diagnostic(
    ...
    /// headless 自动终止：直接终止目标进程并按 Block 决策做完整簿记。
    ///  Headless auto-terminate: terminates the target process and records a Block
    ///  decision with full bookkeeping.
    ///
    /// 终止失败时退回默认回滚（恢复进程 + Allow），避免把进程永久冻结。
    ///  On termination failure it falls back to the default rollback (resume + Allow)
    ///  so the process is never left permanently frozen.
    fn headless_auto_terminate(&self, entry: InterceptionEntry) -> Option<InterceptionEntry> {
        match self.process_control.terminate_process(entry.pid) {
            Ok(_) => {
                append_interception_diagnostic(
                    "headless_auto_terminated",
                    serde_json::json!({
                        "pid": entry.pid,
                        "processName": entry.process_name,
                        "filePath": entry.file_path,
                ...
                eprintln!(
                    "[InterceptionService] Headless auto-terminated PID {} ({})",
                    entry.pid, entry.process_name
                );
                self.mark_decision(entry.pid, InterceptionDecision::Block);
            }
            Err(err) => {
                append_interception_diagnostic(
                    "headless_auto_terminate_failed",
                    serde_json::json!({
                        "pid": entry.pid,
                        "processName": entry.process_name,
                        "error": err,
                ...
                eprintln!(
                    "[InterceptionService] Headless auto-terminate failed for PID {} ({}), falling back to resume+allow",
                    entry.pid, err
                );
                if let Err(resume_err) = self.process_control.resume_process(entry.pid) {
                    append_interception_diagnostic(
                        "headless_auto_terminate_resume_failed",
                        serde_json::json!({
                            "pid": entry.pid,
                            "processName": entry.process_name,
                            "error": resume_err,
                    ...
                    eprintln!(
                        "[InterceptionService] Failed to resume PID {} after headless terminate failure: {}",
                        entry.pid, resume_err
                    );
                }
                self.mark_decision(entry.pid, InterceptionDecision::Allow);
