...
pub mod methods {
    ...
    // ----------------------------------------------------------------
    // 服务生命周期控制 / Service lifecycle control
    // ----------------------------------------------------------------

    /// UI/托盘 → 服务: 请求优雅停止整个防护服务（置位停止标志，由服务主循环收尾）。
    /// 参数：无。响应：`{ "accepted": true }`。
    ///  UI/tray → service: request a graceful full-service stop (sets the stop flag; the
    ///  service main loop performs teardown). Params: none. Response: `{ "accepted": true }`.
    pub const SHUTDOWN_SERVICE: &str = "shutdown_service";
