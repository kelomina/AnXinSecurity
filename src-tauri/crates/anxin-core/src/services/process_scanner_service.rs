...
/// 枚举当前全部 PID（Toolhelp 快照）。
///  Enumerates all current PIDs via a Toolhelp snapshot.
///
/// 供进程扫描循环与 ProcessLifecycleService 的快照纠偏（契约 §4.4）复用。
///  Shared by the scanner loops and ProcessLifecycleService's snapshot
///  reconciliation (contract §4.4).
pub(crate) fn collect_current_pids() -> HashSet<u32> {
