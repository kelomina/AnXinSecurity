...
pub fn build_etw_service_context(app_handle: &tauri::AppHandle) -> ServiceContext {
    ...
    // EtwService 以 Arc<Mutex<EtwService>> 管理（main.rs setup 无条件 manage），
    // 桥接进 ServiceContext 供 ProcessLifecycleService 做 §4.5 ETW 降噪切换。
    //  EtwService is managed as Arc<Mutex<EtwService>> (unconditionally managed in
    //  main.rs setup); bridged into the ServiceContext for the §4.5 ETW reduced-mode switch.
    if let Some(etw) = app_handle.try_state::<Arc<Mutex<crate::services::etw_service::EtwService>>>()
    {
        ctx.register::<Mutex<crate::services::etw_service::EtwService>>(etw.inner().clone());
    }
...
pub fn build_service_context(
    ...
    // 进程监控采集服务（只注册不启动：服务启动流程按 procMonitor.enabled 开关拉起）
    //  Process monitor collector (registered but not started: the service startup
    //  flow brings it up per the procMonitor.enabled switch)
    let process_lifecycle = Arc::new(
        crate::services::process_lifecycle_service::ProcessLifecycleService::new(),
    );
    ctx.register::<crate::services::process_lifecycle_service::ProcessLifecycleService>(
        process_lifecycle,
    );
