...
/// 已知的 AnXinSecurity 可执行文件名（比较前统一小写，Windows 文件名不区分大小写）
///  Known AnXinSecurity executable filenames (compared lowercased; Windows file names
///  are case-insensitive)
///
/// 三进程拆分后合法的 IPC 客户端为 Main 主界面与 Tray 托盘进程；服务进程自身
/// 不会作为客户端连接。Phase 6 若调整 bin 命名必须同步本名单，否则 GUI 进程
/// 会被 fail-closed 拒连、拦截弹窗链路失效。
///  After the three-process split, legitimate IPC clients are the Main UI and the
///  Tray process; the service process never connects as a client. If Phase 6 renames
///  the binaries this list must be updated in lockstep, otherwise GUI processes are
///  rejected fail-closed and the interception-prompt path breaks.
const ANXIN_EXE_NAMES: &[&str] = &[
    // 大驼峰产品命名（Phase 6 起的正式产物名）
    "anxinsecurity.exe",
    "anxintray.exe",
    // 连字符历史名（单 exe 时代与过渡期产物）
    "anxin-security.exe",
    "anxin-tray.exe",
];
