...
pub struct EtwService {
    ...
    /// 当前内核追踪标志（§4.5：驱动接管后降噪可切换；全量默认）
    ///  Current kernel trace flags (§4.5: reduced after driver takeover; full by default)
    enable_flags: Arc<std::sync::atomic::AtomicU32>,
}
...
impl EtwService {
    ...
    pub fn new() -> Arc<Mutex<Self>> {
        ...
        Arc::new(Mutex::new(Self {
            ...
            enable_flags: Arc::new(std::sync::atomic::AtomicU32::new(
    ...
    pub fn start(&self, ctx: ServiceContext) -> Result<(), String> {
        ...
        let mut session = EtwSession::new("AnXinETWSession", flags)?;
    ...
    /// 函数名称：restart_with_flags
    /// 函数作用：以新内核追踪标志重启 ETW 会话（§4.5 降噪切换）。
    /// Purpose: Restarts the ETW session with new kernel trace flags (§4.5 reduced-mode switch).
    ///
    /// 调用方：ProcessLifecycleService（驱动接管成功 → 降噪；驱动失联 → 恢复全量）。
    /// Called by: ProcessLifecycleService (reduced after driver takeover; full on driver loss).
    /// 幂等：当前 flags 与目标一致时直接返回；重启失败返回 Err 并保持旧 flags。
    /// Idempotent: returns immediately when the target flags are already active; on
    /// failure returns Err and keeps the old flags.
    /// 中文关键词：ETW降噪，标志切换，会话重启
    /// English keywords: ETW reduced mode, flag switch, session restart
    pub fn restart_with_flags(&self, ctx: ServiceContext, flags: u32) -> Result<(), String> {
        let current = self.enable_flags.load(std::sync::atomic::Ordering::SeqCst);
        if current == flags {
            return Ok(());
        }

        // 先停掉旧会话（running=false），再以新 flags 启动。
        //  Stop the old session first (running=false), then start with the new flags.
        if self.running.load(std::sync::atomic::Ordering::SeqCst) {
            self.pause()?;
        }
        self.enable_flags.store(flags, std::sync::atomic::Ordering::SeqCst);
        let result = self.start(ctx);
        if result.is_err() {
            // 回滚 flags：新 flags 启动失败时恢复旧值，避免状态与真实会话漂移
            self.enable_flags
    ...
    /// 当前生效的内核追踪标志（诊断用，§4.5） / Current kernel trace flags (diagnostics)
    pub fn current_flags(&self) -> u32 {
        self.enable_flags.load(std::sync::atomic::Ordering::SeqCst)
    }
