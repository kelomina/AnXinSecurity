...
pub struct AppConfig {
    ...
    // 进程监控采集模块（AnXinProcMon.sys）。default 兜底旧版 app.json 缺键。
    //  Process monitor collector (AnXinProcMon.sys). `default` keeps older configs loadable.
    #[serde(rename = "procMonitor", default)]
    pub proc_monitor: ProcMonitorConfig,
    // locale 字段 — 界面语言设置 / Locale field for UI language
    #[serde(default = "default_locale")]
    pub locale: String,
    // headless 自动终止模式（测试 / 无 UI 服务场景专用）。开启后，服务进程在
    // recommendAction=block 的拦截因无 UI 客户端无法弹窗时，直接终止目标进程，
    // 而不是回滚放行。默认关闭（fail-open：无 UI 时「记录在案的放行」）。
    //  Headless auto-terminate mode (test / headless service scenario only). When on,
    //  a recommendAction=block intercept that cannot show a window because no UI
    //  client is connected terminates the target process instead of rolling back
    //  and releasing it. Off by default (fail-open: a logged release).
    #[serde(rename = "headlessAutoTerminate", default)]
    pub headless_auto_terminate: bool,
    // 新增：扫描排除项列表
    // 新增：启动允许列表
}
...
/// 进程监控采集模块（AnXinProcMon.sys）配置。
///  Process monitor collector (AnXinProcMon.sys) configuration.
///
/// 对应 config/app.json 的 `procMonitor` 段（契约 v6 §6）。采集驱动是只读的：
/// 不阻断进程、不拦截行为，仅采集事件供 EDR 研判，因此默认启用；驱动未安装
/// 或加载失败时按功能降级处理，不影响其他防护模块。
///  Backs the `procMonitor` section of config/app.json (contract v6 §6). The
///  collector is read-only: it blocks nothing, only captures events for EDR
///  analysis, so it defaults on; a missing/failed driver degrades the feature
///  without affecting the rest of the suite.
#[derive(Serialize, Deserialize, Clone, Debug)]
#[serde(default)]
pub struct ProcMonitorConfig {
    /// 总开关。关闭时驱动不加载、探针不运行。 / Master switch.
    pub enabled: bool,
    /// 文件行为采集（minifilter）/ File behavior collection (minifilter)
    pub file: bool,
    /// 注册表行为采集（Cm 回调）/ Registry behavior collection (Cm callback)
    pub registry: bool,
    /// 网络行为采集（WFP 只读 callout）/ Network behavior collection (WFP read-only callout)
    pub network: bool,
    /// IPC 行为采集 / IPC behavior collection
    pub ipc: bool,
    /// 驱动在线时收敛 ETW 类别（契约 §4.5）/ Reduce ETW classes while the driver is online
    #[serde(rename = "etwReduced")]
    pub etw_reduced: bool,
}

impl Default for ProcMonitorConfig {
    /// 采集驱动只读不阻断，默认启用；驱动缺失时按降级处理。
    ///  The collector is read-only, so it defaults on; a missing driver is a
...
pub struct BehaviorAnalyzerConfig {
    ...
    /// 行为事件保留天数（events / process_lifecycle 表按此自动清理，防 DB 无限占盘）。
    ///  Event retention days: events / process_lifecycle rows older than this are
    ///  pruned automatically to cap the behavior DB size.
    #[serde(rename = "retentionDays", default = "default_behavior_retention_days")]
    pub retention_days: u64,
    /// 行为库文件大小上限（字节）。超过后定时清理会先删除最老数据并 VACUUM
    /// 回收磁盘，防事件量大的用户一天刷爆占盘。
    ///  Behavior-DB file-size cap in bytes. Once exceeded, the periodic cleaner
    ///  deletes the oldest rows and VACUUMs to reclaim disk, so users with very
    ///  high event rates cannot balloon the DB in a single day.
    #[serde(rename = "maxDbBytes", default = "default_behavior_max_db_bytes")]
    pub max_db_bytes: u64,
    pub sqlite: SqliteConfig,
}

/// 默认行为事件保留天数 / Default behavior-event retention days
fn default_behavior_retention_days() -> u64 {
    3
}

/// 默认行为库大小上限：1 GiB / Default behavior-DB size cap: 1 GiB
fn default_behavior_max_db_bytes() -> u64 {
    1024 * 1024 * 1024
}
    ...
    fn default() -> Self {
        Self {
            ...
            behavior_analyzer: BehaviorAnalyzerConfig {
                ...
                retention_days: default_behavior_retention_days(),
                max_db_bytes: default_behavior_max_db_bytes(),
                sqlite: SqliteConfig {
                    mode: "file".to_string(),
            ...
            proc_monitor: ProcMonitorConfig::default(),
            locale: "zh-CN".to_string(),
            headless_auto_terminate: false,
            // 新增字段默认值
        }
    }
}
...
impl AppConfig {
    ...
    /// 最后尝试可执行文件旁的资源目录（安装包 `_up_\config\app.json` 与 `resources\config\app.json`——
    /// Windows 服务进程的 CWD 是 System32，读不到 CWD/上级配置，必须走资源目录 fallback）。
    /// Purpose: Loads config from config/app.json. Tries CWD first, then parent dir (tauri dev compat),
    /// then the resource dir next to the executable (the packaged `_up_\config\app.json` and
    /// `resources\config\app.json` — a Windows service's CWD is System32, so the packaged path
    /// is the only reliable source).
    /// 中文关键词：配置加载，路径解析，tauri dev，资源目录，服务进程
    /// English keywords: config loading, path resolution, tauri dev, resource dir, service process
    pub fn load() -> Result<Self, Box<dyn std::error::Error>> {
        Self::try_load_path("config/app.json")
            .or_else(|_| Self::try_load_path("../config/app.json"))
            .or_else(|_| Self::try_load_exe_relative("_up_/config/app.json"))
            .or_else(|_| Self::try_load_exe_relative("resources/config/app.json"))
            .or_else(|_| Self::try_load_exe_relative("config/app.json"))
    }

    /// 解析实际加载配置的文件路径（与 load() 的候选顺序一致），供受权 CLI 写入同一路径。
    ///  Resolves the file path that load() actually reads (same candidate order), so an
    ///  authorized CLI subcommand can write back to the exact same file.
    pub fn resolve_load_path() -> Result<PathBuf, Box<dyn std::error::Error>> {
        for candidate in [
        ...
        Err("no config/app.json found".into())
    }


    /// 解析 ETW 规则配置的实际加载路径。优先复用 resolve_load_path() 命中的
    /// app.json 所在目录（保证写入与 app.json 同目录），其次按相同候选
    /// 顺序查找已存在的 etw_match_rules.json。若所有路径均不存在则回落到 app.json 同目录。
    pub fn resolve_etw_rules_path() -> Result<PathBuf, Box<dyn std::error::Error>> {
        if let Ok(app_path) = Self::resolve_load_path() {
            if let Some(dir) = app_path.parent() {
                let candidate = dir.join("etw_match_rules.json");
                if candidate.exists() { return Ok(candidate); }
                return Ok(candidate);
            }
        }
        for candidate in ["config/etw_match_rules.json", "../config/etw_match_rules.json"] {
            let p = PathBuf::from(candidate); if p.exists() { return Ok(p); }
        }
        for candidate in ["_up_/config/etw_match_rules.json", "resources/config/etw_match_rules.json", "config/etw_match_rules.json"] {
            let exe = std::env::current_exe()?;
            let dir = exe.parent().ok_or("cannot resolve executable directory")?;
            let p = dir.join(candidate); if p.exists() { return Ok(p); }
        }
        let exe = std::env::current_exe()?;
        let dir = exe.parent().ok_or("cannot resolve exe dir")?;
        Ok(dir.join("config").join("etw_match_rules.json"))
    }

    /// 以受权方式写入 ETW 规则 JSON（anxin-security.exe 进程可通过 FileProtect 校验）。
    pub fn write_etw_rules(src: &std::path::Path) -> Result<PathBuf, Box<dyn std::error::Error>> {
        use std::io::Read;
        let path = Self::resolve_etw_rules_path()?;
    ...
    /// 以受权方式写入单个配置字段（对抗测试部署用）。由 anxin-security.exe 自身进程
    /// 调用时通过 FileProtect 的 IsCallerAuthorized 校验，与 --write-etw-rules 同机制。
    /// 支持点号分隔的嵌套路径（如 procMonitor.etwReduced、behaviorAnalyzer.sqlite.mode），
    /// 中间不存在的对象容器会自动创建。
    ///  Writes a single config field through the authorized path (antagonist-test deploy).
    ///  Invoked by the anxin-security.exe process itself, passing FileProtect's
    ///  IsCallerAuthorized check (same mechanism as --write-etw-rules).
    ///  Supports dot-separated nested paths (e.g. procMonitor.etwReduced); intermediate
    ///  object containers are created if missing.
    pub fn set_cli_value(key: &str, value: &str) -> Result<PathBuf, Box<dyn std::error::Error>> {
        ...
        // Navigate into nested objects, creating intermediate maps if needed.
        // For n parts, the first n-1 parts are navigated; the last is the leaf key.
        ...
        for i in 0..parts.len().saturating_sub(1) {
            ...
            if !cursor.is_object() {
                return Err(format!("key segment '{}' is not an object in '{}'", segment, key).into());
            }
            ...
            cursor = obj.get_mut(segment).unwrap();
        }
        ...
        match cursor.as_object_mut() {
            Some(obj) => { obj.insert(leaf.to_string(), parsed); }
            None => return Err(format!("key '{}' does not resolve to an object container", key).into()),
        }
    ...
    /// 从可执行文件所在目录的相对路径加载配置（服务进程等 CWD 不可靠的场景）。
    ///  Loads config from a path relative to the executable directory (for scenarios
    ///  where the CWD is unreliable, e.g. the Windows service process).
    fn try_load_exe_relative(relative: &str) -> Result<Self, Box<dyn std::error::Error>> {
        let exe = std::env::current_exe()?;
        let dir = exe
            .parent()
            .ok_or("cannot resolve executable directory")?;
